...
import { BookOpen, CheckCircle, Lock, Unlock, Play, FileText, Award, Clock, ChevronLeft, ChevronRight, Trophy, Star, Crown, ThumbsUp, ThumbsDown, FileDown } from 'lucide-react';
...
export default function CourseLearningPage() {
  ...
  const [modulesCollapsed, setModulesCollapsed] = useState(() => {
    const saved = localStorage.getItem('modulesColumnCollapsed');
  ...
  useEffect(() => {
    localStorage.setItem('modulesColumnCollapsed', JSON.stringify(modulesCollapsed));
  }, [modulesCollapsed]);
  ...
  // Función para verificar si todas las lecciones previas del módulo están completadas
  const areAllPreviousLessonsCompleted = (): { allCompleted: boolean; pendingLessons: string[] } => {
    if (!course || !selectedLesson) return { allCompleted: true, pendingLessons: [] };

    for (const module of course.modules) {
      const lessonIndex = module.lessons.findIndex(l => l.id === selectedLesson.id);
      if (lessonIndex !== -1) {
        // Verificar todas las lecciones anteriores del módulo
        const pendingLessons: string[] = [];
        for (let i = 0; i < lessonIndex; i++) {
          if (!completedLessons.has(module.lessons[i].id)) {
            pendingLessons.push(module.lessons[i].title);
          }
        }
        return {
          allCompleted: pendingLessons.length === 0,
          pendingLessons
        };
      }
    }
    return { allCompleted: true, pendingLessons: [] };
  };
  ...
  const markLessonAsCompleted = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Mostrar celebración pero NO navegar automáticamente
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-0 h-[calc(100vh-8rem)] bg-white dark:bg-gray-950 relative">
      ...
      {/* Modal de Lecciones Pendientes */}
      {showPendingLessonsAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 max-w-md mx-4 text-center">
            <div className="mb-6">
              <Lock className="w-16 h-16 mx-auto text-yellow-600 dark:text-yellow-500" />
            </div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">
              Lecciones Pendientes
            </h2>
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Para acceder al test de evaluación, primero debes completar las siguientes lecciones:
            </p>
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-6 max-h-60 overflow-y-auto">
              <ul className="text-left space-y-2">
                {pendingLessonsList.map((lessonTitle, index) => (
                  <li key={index} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                    <CheckCircle className="w-4 h-4 mt-0.5 text-gray-400 flex-shrink-0" />
            ...
            <button
              onClick={() => setShowPendingLessonsAlert(false)}
      ...
      <div className={`flex-shrink-0 bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 overflow-hidden transition-all duration-300 ${modulesCollapsed ? 'w-16' : 'lg:w-80 w-full'}`}>

        {/* Header del curso */}
        <div className={`border-b border-gray-200 dark:border-gray-800 transition-all duration-300 ${modulesCollapsed ? 'p-2' : 'p-6'}`}>
          {!modulesCollapsed && (
            <>
          ...
          <button
            ...
            className={`flex items-center w-full py-2 rounded-lg transition-all duration-200 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 ${modulesCollapsed ? 'justify-center px-2 mt-0' : 'justify-start px-4 mt-4'}`}
            title={modulesCollapsed ? 'Expandir módulos' : 'Contraer módulos'}
          >
            {modulesCollapsed ? (
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 16rem)' }}>
          {course.modules.map((module, moduleIndex) => (
            <div key={module.id} className="border-b border-gray-100 dark:border-gray-900">
              ...
              {/* Header de módulo */}
              <div className={`bg-gray-50 dark:bg-gray-900/50 transition-all duration-300 ${modulesCollapsed ? 'p-2' : 'p-4'}`}>
                {modulesCollapsed ? (
                  <div className="flex flex-col items-center">
                    ...
                    {moduleGrades.has(module.id) && (
                      <span className={`text-xs mt-1 ${getGradeDisplay(moduleGrades.get(module.id)!).color}`}>
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-2 mt-1">
                      ...
                      {/* Botón de descarga de PDF para módulos aprobados */}
                      {moduleGrades.has(module.id) && moduleGrades.get(module.id)! >= 5.0 && (
                        <button
                          onClick={async (e) => {
                            ...
                            try {
                              const token = localStorage.getItem('token');
                              const response = await fetch(`http://localhost:3001/api/pdf/module/${module.id}/download`, {
                                headers: {
                                  'Authorization': `Bearer ${token}`,
                                },
                              });
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-orange-600 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded transition-colors"
                          title="Descargar PDF del módulo"
                        >
                ...
                {module.lessons.map((lesson, lessonIndex) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={blocked}
                      title={modulesCollapsed ? lesson.title : undefined}
                      className={`
                        w-full text-left transition-colors duration-150
                        ${modulesCollapsed ? 'p-2' : 'p-4'}
...
{(() => {
                  ...
                  const { allCompleted, pendingLessons } = areAllPreviousLessonsCompleted();

                  // Si es la última lección del módulo y está completada
                  if (isLast && isCompleted && moduleTestId) {
                    // Verificar que TODAS las lecciones previas estén completadas
                    if (allCompleted) {
                      // Mostrar botón "Test de evaluación"
                      return (
                        <button
                      ...
                      // Mostrar botón de aviso de lecciones pendientes
                      return (
                        <button
                          onClick={() => {
                            setShowPendingLessonsAlert(true);
                            setPendingLessonsList(pendingLessons);
                          }}
                          className="flex items-center gap-2 px-6 py-2 text-white text-sm font-medium transition-colors bg-yellow-600 hover:bg-yellow-700"
                        >
                          <Award className="w-4 h-4" />

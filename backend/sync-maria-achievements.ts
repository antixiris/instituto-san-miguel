...
import { checkAllAchievements } from './src/services/gamificationService';
...
async function syncMariaAchievements() {
  ...
  try {
    ...
    if (achievementsAfter.length > 0) {
      // Agrupar por tipo
      const byType = achievementsAfter.reduce((acc: any, ua) => {
      ...
      Object.entries(byType).forEach(([type, achievements]: [string, any]) => {

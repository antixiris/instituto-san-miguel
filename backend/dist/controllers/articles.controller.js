...
async function getAllArticles(req, res, next) {
    try {
        const { page = '1', limit = '12', search, category, tag, featured, author, } = req.query;
        ...
        if (!req.user || (req.user.role !== client_1.UserRole.ADMIN && req.user.role !== client_1.UserRole.PROFESOR)) {
...
async function getArticleBySlug(req, res, next) {
    try {
        ...
        if (!article) {
            throw new errors_1.NotFoundError('Artículo no encontrado');
        }
        // Verificar si el artículo está publicado
        if (!article.published &&
            (!req.user ||
                (req.user.role !== client_1.UserRole.ADMIN &&
                    req.user.role !== client_1.UserRole.PROFESOR &&
...
async function createArticle(req, res, next) {
    try {
        ...
        const { title, subtitle, excerpt, content, coverImage, categoryId, featured, published, tags, footnotes, } = req.body;
        // Generar slug único
        ...
        const article = await prisma_1.prisma.article.create({
            data: {
                ...
                footnotes: footnotes ? {
                    create: footnotes.map((note, index) => ({
...
async function updateArticle(req, res, next) {
    try {
        ...
        const existingArticle = await prisma_1.prisma.article.findUnique({
            where: { id },
        });
        if (!existingArticle) {
            throw new errors_1.NotFoundError('Artículo no encontrado');
        ...
        if (req.user.role !== client_1.UserRole.ADMIN &&
            ...
            throw new errors_1.AuthorizationError('No tienes permiso para editar este artículo');
        }
        ...
        if (updateData.tags) {
            ...
            await prisma_1.prisma.articleTag.deleteMany({
                where: { articleId: id },
            });
            // Crear nuevos tags
            updateData.tags = {
        ...
        if (updateData.footnotes) {
            ...
            await prisma_1.prisma.articleFootnote.deleteMany({
                where: { articleId: id },
            });
            // Crear nuevos footnotes
            updateData.footnotes = {
                create: updateData.footnotes.map((note, index) => ({
                    number: note.number || index + 1,
                    content: note.content,
                    order: index,
                })),
...
async function deleteArticle(req, res, next) {
    try {
        ...
        if (req.user.role !== client_1.UserRole.ADMIN &&
            ...
            throw new errors_1.AuthorizationError('No tienes permiso para eliminar este artículo');
        }
        // Eliminar artículo (Prisma eliminará automáticamente las relaciones)
        await prisma_1.prisma.article.delete({
            where: { id },
        });
        const response = {
            success: true,
...
async function getArticleCategories(req, res, next) {
    try {
        const categories = await prisma_1.prisma.articleCategory.findMany({
            orderBy: {
                order: 'asc',
            },
            include: {
 ...
 */
async function getArticleTags(req, res, next) {
    try {
        const tags = await prisma_1.prisma.articleTagName.findMany({
            orderBy: {
                name: 'asc',
            },
            include: {
